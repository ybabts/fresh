...
function useLemonDropAnim() {
  ...
  return { dropy, juice, springsPath };
}

function LemonDrop(
  { isHome = false, tankWidth = "100%", dropHeight = "300px" }: {
    isHome?: boolean;
    tankWidth?: string;
    dropHeight?: string;
  },
) {
  const { dropy, juice, springsPath } = useLemonDropAnim();
  ...
  return (
    <>
      ...
      >
        <circle cx="18" cy={dropy.value} r="4" fill="white">
        </circle>
        {!isHome && (
          <>
            <path
